...
def imprimirMensajeCerca(min1,min2,baseDatos):
    ...
    if baseDatos[min1][2] > baseDatos[min2][2]:
        ...
        if opcionDestino==1:
            ...
            informacion["zonawifi1"] = [baseDatos[min1][0],baseDatos[min1][1],baseDatos[min1][2]]
            indicaciones(baseDatos[min1])
        elif opcionDestino==2:
            distanciaparatiempo = listaDistancias[min2]
            informacion["zonawifi1"] = [baseDatos[min2][0],baseDatos[min2][1],baseDatos[min2][2]]
...
def calcularTiempo():
    ...
    if distanciaparatiempo == 0:
        ...
        bus = round(bus)
        bici = round(bici)
        print(f"Se tardará aproximadamente {bus} {tiempo1} en bus y {bici} {tiempo2} en bicicleta")

        tmp1 = distanciaparatiempo
        tmp2 = "En bus tardarías"
        tmp3 = f"{bus} {tiempo1}"
        tmp4 = "y en bicicleta tardarías"
        tmp5 = f"{bici} {tiempo2}"
...
def leerArchivo(archivo):
    try:
        ...
        for x in range(0,4):
            listaTempCoord.append([])
            tmp = lineaArchivo[x].split(",")
            for y in range(0,3):
                listaTempCoord[x].append(tmp[y])
        
        for x in range(len(listaTempCoord)):
            for y in range(len(listaTempCoord[x])):
                listaTempCoord[x][y] = float(listaTempCoord[x][y])
                if y == 2:
                    listaTempCoord[x][y] = int(listaTempCoord[x][y])
        print("Coordenadas actualizadas")
        return listaTempCoord
    except IOError:
        print("Error creación de fichero")
        return[[6.211,-72.482,2],[6.212,-72.470,25],[6.105,-72.342,25],[6.210,-72.442,50]]
...
def crearArchivo():
    if alistamiento:
        try:
            val=input(" Está de acuerdo con la información a exportar? Presione 1 para confirmar, 0 para regresar al men  principal")
            if val == 1:
                archivo = open("archivo2.txt","w",encoding="utf-8")
            ...
            while True:
                ...
                try:
                    ...
                    if opcion < 1 or opcion > 7:
                        ...
                        listaPredefinida = (leerArchivo("archivo.txt"))
                    elif opcion == 6:
                        try:
